...
<title>Daftar Nilai Skripsi Mahasiswa</title>
<!-- /. ROW  -->
						...
						if(isset($_POST['search_tgl'])) {
							if(empty($dari_tgl) && empty($sampai_tgl)){
								if($periode=='-Pilih Periode-'){
									$show_pendaftar="SELECT mahasiswa.nama, uji_skripsi.* from uji_skripsi inner join mahasiswa on mahasiswa.nim=uji_skripsi.nim where is_test='sudah'";
								}else{
									$show_pendaftar = "SELECT mahasiswa.nama, uji_skripsi.* from uji_skripsi inner join mahasiswa on mahasiswa.nim=uji_skripsi.nim where is_test='sudah' and daftar_skripsi.tahun_ajaran='$periode'";
								}
							}else{
								$show_pendaftar = "SELECT mahasiswa.nama, uji_skripsi.* from uji_skripsi inner join mahasiswa on mahasiswa.nim=uji_skripsi.nim where is_test='sudah' and (cast(uji_skripsi.jadwal as date) BETWEEN '$dari_tgl' AND '$sampai_tgl') ";
							}
						}else{
							$show_pendaftar="SELECT mahasiswa.nama, uji_skripsi.* from uji_skripsi inner join mahasiswa on mahasiswa.nim=uji_skripsi.nim where is_test='sudah'";
						}
							...
							while($rshow_pendaftar = $fshow_pendaftar->fetch_object()){
								...
								echo "<td id='tools' align='center'>
									<a href='tampilkan_nilai_skripsi.php?nim=".$rshow_pendaftar->nim."&id=".$rshow_pendaftar->id_uji_skripsi."'><i class='fa fa-eye'></i> Tampilkan Nilai</a>
								 </td>";
