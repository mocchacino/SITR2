<?php	
	...
	if (!isset($_SESSION['sip_masuk_aja'])){
		...
		if ($_SERVER['REQUEST_METHOD'] === 'POST'){
			// $nim='24010314120014';
			$nim = $_POST['nim'];
			$query = mysqli_query($con,"SELECT mahasiswa.nama, jadwal, tempat, nip1, nip2, nip3, judul FROM daftar_skripsi inner join mahasiswa on mahasiswa.nim=daftar_skripsi.nim inner join uji_skripsi on uji_skripsi.nim=mahasiswa.nim inner join penguji_skripsi on penguji_skripsi.id_uji_skripsi=uji_skripsi.id_uji_skripsi  where mahasiswa.nim='$nim' ");
			$row = $query->fetch_row();
			...
			$penguji = mysqli_query($con,"SELECT dosen.nip, dosen.nama_dosen FROM dosen inner join penguji_skripsi on penguji_skripsi.nip_penguji_skripsi=dosen.nip inner join uji_skripsi on uji_skripsi.id_uji_skripsi=penguji_skripsi.id_uji_skripsi where uji_skripsi.nim='$nim' and jabatan != 'sekretaris' order by jabatan"); 
