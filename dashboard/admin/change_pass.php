	...
	if (!isset($_SESSION['sip_masuk_aja'])){
		...
		if ($_SERVER['REQUEST_METHOD'] === 'POST'){
			if (isset($_POST['submit'])) {
				$pass = mysqli_query($con, "SELECT * FROM petugas where idpetugas='".$_SESSION['sip_masuk_aja']."' ");
				...
				if($validPassBaru && $validPassLama && $validPassBaruConfirm){
			  		$change_pass = mysqli_query($con,"UPDATE petugas SET password='".md5('sip'.$new_pass.'pis')."' where idpetugas='".$_SESSION['sip_masuk_aja']."' ");
