<?php		
	...
	if (!isset($_SESSION['sip_masuk_aja'])){
		...
		$show_detail=mysqli_query($con, "SELECT daftar_skripsi.*, mahasiswa.nama FROM daftar_skripsi INNER JOIN mahasiswa on mahasiswa.nim=daftar_skripsi.nim where daftar_skripsi.id_daftar_skripsi='$id' and daftar_skripsi.nim='$nim' ORDER BY daftar_skripsi.tgl_daftar ");	
						...
						<div class="col-md-12">
							<form>
								...
								<div class="row">
									...
									<div class="form-group col-md-4 col-sm-4 col-xs-12">
										<input class="form-control" name="tgl_lulus" id="tgl_lulus" value="<?php if($rshow_detail['tgl_lulus']!=0000-00-00) { echo $rshow_detail['tgl_lulus'];}else{echo "Belum/Tidak Lulus";}?>" readonly>
									</div>
									<br><br>
								
								</div>
								<?php
									echo '<a href="edit_mahasiswa_skripsi.php?nim='.$nim.'&id='.$id.'" class="btn btn-primary btn-sm pull-right"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Edit</a>';
								?>
							</form>
						</div>
					</div>
