<?php		
	...
	if (!isset($_SESSION['sip_masuk_aja'])){
		...
		$dari_tgl='';
		$sampai_tgl='';
		$periodeTersedia=mysqli_query($con, "SELECT distinct tahun_ajaran from daftar_tugas_riset2 order by tahun_ajaran desc");
?>
...
<script>
	...
	$(document).ready(function(){
		...
		$('#print').click(function(){
			...
			var kopsurat = '<body><h1 style="text-align:center;">DAFTAR TUGAS RISET II<br>DEPARTEMEN KIMIA FAKULTAS SAINS DAN MATEMATIKA<br>UNIVERSITAS DIPONEGORO SEMARANG</h1><hr><hr><br> <script type="text/javascript" src="https://code.jquery.com/jquery-3.3.1.min.js"><\/script>';
				...
				<div class="col-md-2 col-sm-12 col-xs-12">
					<div class="form-group">
						...
						<!-- <a href="daftar_uji_kelayakan.php">
						 ...
						 -->
						 <input class="form-control btn btn-danger" type="button" name="reset" value="Reset" onclick="window.location.href='admin_daftar_uji_kelayakan.php'" />
					...
					<table id ="myTable" class="display" style="width: 100%">
						...
						if(isset($_POST['search_tgl'])) {
							if($periode!='--Pilih Periode--'){
								$show_pendaftar = "SELECT daftar_tugas_riset2.*, mahasiswa.nama, lab.nama_lab FROM daftar_tugas_riset2 INNER JOIN mahasiswa on mahasiswa.nim=daftar_tugas_riset2.nim inner join tr1 on tr1.nim=daftar_tugas_riset2.nim inner join lab on lab.idlab=tr1.idlab_tr1 where daftar_tugas_riset2.tahun_ajaran='$periode' ORDER BY daftar_tugas_riset2.tgl_daftar";
							}elseif(($dari_tgl!='')&&($sampai_tgl!='')){
								$show_pendaftar = "SELECT daftar_tugas_riset2.*, mahasiswa.nama, lab.nama_lab FROM daftar_tugas_riset2 INNER JOIN mahasiswa on mahasiswa.nim=daftar_tugas_riset2.nim inner join tr1 on tr1.nim=daftar_tugas_riset2.nim inner join lab on lab.idlab=tr1.idlab_tr1 where daftar_tugas_riset2.tgl_daftar BETWEEN '$dari_tgl' AND '$sampai_tgl' ORDER BY daftar_tugas_riset2.tgl_daftar";
							}
						}else{
							$show_pendaftar = "SELECT daftar_tugas_riset2.*, mahasiswa.nama, lab.nama_lab FROM daftar_tugas_riset2 INNER JOIN mahasiswa on mahasiswa.nim=daftar_tugas_riset2.nim inner join tr1 on tr1.nim=daftar_tugas_riset2.nim inner join lab on lab.idlab=tr1.idlab_tr1 ORDER BY daftar_tugas_riset2.tgl_daftar";
						}
							...
							while($rshow_pendaftar = $fshow_pendaftar->fetch_object()){
								...
								$nama_pembimbing1=mysqli_query($con,"SELECT * FROM dosen WHERE dosen.nip='".$rshow_pendaftar->nip1."' ");
								
								while($rnama_pembimbing1=$nama_pembimbing1->fetch_object()){
								...
								<a href='detail_mhs_tr2.php?nim=".$rshow_pendaftar->nim."' title='Detail'><i class='fa fa-info-circle'></i></a>
									<a href='edit_mahasiswa_tr2.php?nim=".$rshow_pendaftar->nim."' title='Edit'><i class='fa fa-edit'></i></a>
									<a data-toggle='modal' 
										data-target='#myModal' 
