<?php		
	...
	if (!isset($_SESSION['sip_masuk_aja'])){
		...
		$show_detail=mysqli_query($con, "SELECT mahasiswa.nama, uji_skripsi.*, judul, nip1, nip2, nip3, tgl_lulus from uji_skripsi inner join mahasiswa on mahasiswa.nim=uji_skripsi.nim inner join daftar_skripsi on daftar_skripsi.id_daftar_skripsi=uji_skripsi.id_uji_skripsi where uji_skripsi.nim='$nim' and uji_skripsi.id_uji_skripsi='$id'");	
		$rshow_detail=$show_detail->fetch_assoc();	
		$show_penguji=mysqli_query($con, "SELECT dosen.nama_dosen, penguji_skripsi.nilai from penguji_skripsi inner join uji_skripsi on uji_skripsi.id_uji_skripsi=penguji_skripsi.id_uji_skripsi inner join dosen on dosen.nip=penguji_skripsi.nip_penguji_skripsi where uji_skripsi.nim='$nim' and uji_skripsi.id_uji_skripsi='$id' and jabatan != 'sekretaris' order by jabatan");
		$nilai_pembimbing1=mysqli_query($con, "SELECT penguji_skripsi.nilai from penguji_skripsi inner join uji_skripsi on uji_skripsi.id_uji_skripsi=penguji_skripsi.id_uji_skripsi where uji_skripsi.nim='$nim' and uji_skripsi.id_uji_skripsi='$id' and nip_penguji_skripsi = '".$rshow_detail['nip1']."' ");
		$rnilai_pembimbing1=$nilai_pembimbing1->fetch_object();
		$nilai_pembimbing2=mysqli_query($con, "SELECT penguji_skripsi.nilai from penguji_skripsi inner join uji_skripsi on uji_skripsi.id_uji_skripsi=penguji_skripsi.id_uji_skripsi where uji_skripsi.nim='$nim' and uji_skripsi.id_uji_skripsi='$id' and nip_penguji_skripsi = '".$rshow_detail['nip2']."' ");
		...
		<div class="col-md-12 col-sm-12 col-xs-12">
			...
			<div class="panel panel-default">
				<div class="panel-heading">
					...
					Detail Nilai Ujian Tugas Akhir Mahasiswa 
				</div>
...
?>	
								<br><br>
								<?php
									echo '<a href="edit_nilai_skripsi.php?nim='.$nim.'&id='.$id.'" class="btn btn-primary btn-sm pull-right"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Edit Nilai</a>';
								?>
