...
<script>
	...
	$(document).ready(function(){
		...
		$('#print').click(function(){
			...
			var kopsurat = '<body><h1 style="text-align:center;">DAFTAR MAHASISWA SUDAH UJI KELAYAKAN<br>DEPARTEMEN KIMIA FAKULTAS SAINS DAN MATEMATIKA<br>UNIVERSITAS DIPONEGORO SEMARANG</h1><hr><hr><br> <script type="text/javascript" src="https://code.jquery.com/jquery-3.3.1.min.js"><\/script>';
			...
			<div class="col-md-2 col-sm-12 col-xs-12">
				<div class="form-group">
					...
					<!-- <a href="daftar_uji_kelayakan.php">
					 ...
					 -->
					 <input class="form-control btn btn-danger" type="button" name="reset" value="Reset" onclick="window.location.href='daftar_nilai.php'" />
					...
					<table id ="myTable" class="display printArea" style="width: 100%">
						...
						if(isset($_POST['search_tgl'])) {
							if(empty($dari_tgl) && empty($sampai_tgl)){
								if(empty($periode)){
									$show_pendaftar="SELECT DISTINCT mahasiswa.nim, mahasiswa.nama,jadwal, tempat,id_uji_kelayakan,is_lulus, is_test from daftar_uji_kelayakan inner join uji_kelayakan on uji_kelayakan.id_uji_kelayakan=daftar_uji_kelayakan.id_daftar_uji_kelayakan inner join mahasiswa on mahasiswa.nim=daftar_uji_kelayakan.nim inner join tr1 on tr1.nim=daftar_uji_kelayakan.nim where tr1.idlab_tr1='$idlab' and is_test='sudah'";
								}else{
									$show_pendaftar = "SELECT DISTINCT mahasiswa.nim, mahasiswa.nama,jadwal, tempat,id_uji_kelayakan,is_lulus, is_test from daftar_uji_kelayakan inner join uji_kelayakan on uji_kelayakan.id_uji_kelayakan=daftar_uji_kelayakan.id_daftar_uji_kelayakan inner join mahasiswa on mahasiswa.nim=daftar_uji_kelayakan.nim inner join tr1 on tr1.nim=daftar_uji_kelayakan.nim where tr1.idlab_tr1='$idlab' and is_test='sudah' and daftar_uji_kelayakan.tahun_ajaran='$periode' ";
								}
							}else{
								$show_pendaftar = "SELECT DISTINCT mahasiswa.nim, mahasiswa.nama,jadwal, tempat,id_uji_kelayakan,is_lulus, is_test from daftar_uji_kelayakan inner join uji_kelayakan on uji_kelayakan.id_uji_kelayakan=daftar_uji_kelayakan.id_daftar_uji_kelayakan inner join mahasiswa on mahasiswa.nim=daftar_uji_kelayakan.nim inner join tr1 on tr1.nim=daftar_uji_kelayakan.nim where tr1.idlab_tr1='$idlab' and is_test='sudah' and (cast(uji_kelayakan.jadwal as date) BETWEEN '$dari_tgl' AND '$sampai_tgl')";
							}
						}else{
							$show_pendaftar="SELECT DISTINCT mahasiswa.nim, mahasiswa.nama,jadwal, tempat,id_uji_kelayakan,is_lulus, is_test from daftar_uji_kelayakan inner join uji_kelayakan on uji_kelayakan.id_uji_kelayakan=daftar_uji_kelayakan.id_daftar_uji_kelayakan inner join mahasiswa on mahasiswa.nim=daftar_uji_kelayakan.nim inner join tr1 on tr1.nim=daftar_uji_kelayakan.nim where tr1.idlab_tr1='$idlab' and is_test='sudah' ";
							...
							while($rshow_pendaftar = $fshow_pendaftar->fetch_object()){
								...
								echo "<td class='tools' align='center'>
									<a href='tampilkan_nilai.php?nim=".$rshow_pendaftar->nim."&id=".$rshow_pendaftar->id_uji_kelayakan."'><i class='fa fa-eye'></i> Tampilkan Nilai</a>
								 </td>";
