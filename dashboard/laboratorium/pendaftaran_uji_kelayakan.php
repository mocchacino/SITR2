<?php
	...
	if (!isset($_SESSION['sip_masuk_aja'])){
		...
		$sukses=TRUE;
		$get_nim=mysqli_query($con,"SELECT daftar_tugas_riset2.nim from daftar_tugas_riset2 inner join tr1 on tr1.nim=daftar_tugas_riset2.nim where idlab_tr1='$idlab' and daftar_tugas_riset2.nim not in (SELECT daftar_uji_kelayakan.nim from daftar_uji_kelayakan inner JOIN uji_kelayakan on uji_kelayakan.nim=daftar_uji_kelayakan.nim where is_lulus='1' and jadwal is not null)" );
							...
							if(isset($_POST['no_nilai_d']) || $_POST['no_nilai_d'] == 'no_nilai_d'){
								if(isset($_POST['toefl']) || $_POST['toefl'] == 'toefl'){
									...
									if($ipk == ''){
										...
										if(is_numeric($ipk)){
											$ipk=floatval($ipk);
											$rSyaratIpk=floatval($rSyaratIpk['syarat_ipk']);
											//$minIpk=$fSyaratDaftar->syarat_ipk;
											if(($ipk>=$rSyaratIpk)&&($ipk<=4)){
												$validIpk=true;
											}else{
												$errorIpk='Nilai IPK tidak memenuhi syarat minimal '.$rSyaratIpk;
												$validIpk=false;
											}
										}else{
									...
									if($validNim&&$validKomulatif&&$validKrs&&$validSks&&$validSemester&&$validIpk&&$validTranskrip){
