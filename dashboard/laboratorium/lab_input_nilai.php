	...
	if (!isset($_SESSION['sip_masuk_aja'])){
		...
		if ($_SERVER['REQUEST_METHOD'] === 'POST') {
			if (isset($_POST['submit'])) {
				...
				$nipPembimbing1=test_input($_POST['nipPembimbing1']);
				$nipPembimbing2=test_input($_POST['nipPembimbing2']);

				$nilai_pembimbing_uji_kelayakan1 = test_input($_POST['nilai_pembimbing_uji_kelayakan1']);
				$nilai_pembimbing_uji_kelayakan2 = test_input($_POST['nilai_pembimbing_uji_kelayakan2']);
				...
				if(isset($is_lulus)){
					...
					for($i =1; $i<=2; $i++){
						if($_POST['nilai_pembimbing_uji_kelayakan'.$i]){
							$counter++;
							${'nipPenguji'.$counter}=test_input($_POST['nipPembimbing'.$i]);
						} 
						if(${"nilai_pembimbing_uji_kelayakan".$i} !=''){
							if (is_numeric(${"nilai_pembimbing_uji_kelayakan".$i})){
								${"nilaiFloat".$counter}=floatval(${"nilai_pembimbing_uji_kelayakan".$i});
								if ((${"nilaiFloat".$counter} >= 0) && (${"nilaiFloat".$counter} <= 100)){
									${"valid_nilai_pembimbing_uji_kelayakan".$i}=$validNilai++;
								}else{
									${"error_nilai_pembimbing_uji_kelayakan".$i} = 'isi dengan nilai diantara 0-100, nilai desimal dipisah dengan titik(.)';
								}
							}else{
								${"error_nilai_pembimbing_uji_kelayakan".$i} = 'isi dengan angka';
							}
						}
					}
				}else{
				...
				if(($counter==$validNilai)&&($counter==$jml_penguji) && $validId){
					...
					if($tambah_nilai && $ubah_is_lulus){
						$sukses=true;
						$pesan_sukses="Berhasil memasukkan data nilai & keterangan layak/tidak";
					}else{
						$sukses=false;
		...
		$(document).ready(function(){
			$('#input_id').change(function(){
				...
				$.ajax({
				    type: "POST",
				    data: "id="+ id,
					url:"autopopulate_lab_input_nilai.php",
					dataType:"json",
					cache: false,
				    success: function(response){
						...
						$('#pembimbing_uji_kelayakan1').val(response.pembimbing1);
						$('#pembimbing_uji_kelayakan2').val(response.pembimbing2);
						if(!response.nipPembimbing2) document.getElementById('nilai_pembimbing_uji_kelayakan2').readOnly=true;
					},
		...
		<div class="col-md-12 col-sm-12 col-xs-12">
			...
			<div class="panel panel-default">
				<div class="panel-heading">
					<a href="index.php" class="btn btn-primary"><i class="fa fa-chevron-circle-left"></i> Kembali</a>
...
?>
								...
								<div class="row">
									<div class="form-group col-md-8 col-sm-8 col-xs-12">
										<label>Pembimbing 1</label>&nbsp;<span class="label label-warning">*<?php if(isset($error_nilai_pembimbing_uji_kelayakan1)) echo $error_nilai_pembimbing_uji_kelayakan1;?></span>
										<input class="form-control" name="pembimbing_uji_kelayakan1" id="pembimbing_uji_kelayakan1" readonly>
									</div>
									<div class="form-group col-md-4 col-sm-4 col-xs-12">
										<label>Nilai</label>&nbsp;
										<input class="form-control" name="nilai_pembimbing_uji_kelayakan1" id="nilai_pembimbing_uji_kelayakan1">
									</div>
								</div>
								<div class="row">
									<div class="form-group col-md-8 col-sm-8 col-xs-12">
										<label>Pembimbing 2</label>&nbsp;<span class="label label-warning">*<?php if(isset($error_nilai_pembimbing_uji_kelayakan2)) echo $error_nilai_pembimbing_uji_kelayakan2;?></span>
										<input class="form-control" name="pembimbing_uji_kelayakan2" id="pembimbing_uji_kelayakan2" readonly>
									</div>
									<div class="form-group col-md-4 col-sm-4 col-xs-12">
										<label>Nilai</label>&nbsp;
										<input class="form-control" name="nilai_pembimbing_uji_kelayakan2" id="nilai_pembimbing_uji_kelayakan2">
									</div>
								</div>
								<div class="row">
									<div class="form-group col-md-8 col-sm-8 col-xs-12">
