	...
	if (!isset($_SESSION['sip_masuk_aja'])){
		...
		switch ($idlab) {
			case '1':
				...
				break;
			case '2':
				$namaLab='KIMIA ANALITIK';
				break;
			case '3':
				$namaLab='KIMIA ANORGANIK';
				break;
			case '4':
				$namaLab='KIMIA FISIK';
				break;
			case '5':
				$namaLab='KIMIA ORGANIK';
				break;
			default:
				header("Location:../login/login.php");
				break;
		...
		$get_nim=$_GET['nim'];
		$id=$_GET['id'];
		$show_pendaftar = mysqli_query($con,"SELECT * FROM daftar_uji_kelayakan INNER JOIN mahasiswa on mahasiswa.nim=daftar_uji_kelayakan.nim inner join daftar_tugas_riset2 on daftar_tugas_riset2.nim=daftar_uji_kelayakan.nim where daftar_uji_kelayakan.nim='$get_nim' and id_daftar_uji_kelayakan='$id' ");
		...
		$jml_infoUjiKelayakan=mysqli_num_rows($infoUjiKelayakan);
		$kalab=mysqli_query($con, "SELECT * from dosen inner join lab on lab.nip=dosen.nip where lab.idlab='$idlab' ");
	...
	<script>
		$(document).ready(function(){
			$('#print').click(function(){
				...
				var win = window.open('');
				win.document.write('<style>div.margin{width: 21cm;min-height: 29.7cm;padding: 0.5cm;} .ttd{padding:0 0 0 175px; width:50%;} .pengisi{width:25%;} table.isi{line-height: 25px; font-size: 18px} .footer_surat{font-size:10px; position:fixed; bottom:50; width: 21cm;} .page-break{page-break-after: always;} .ket_lembar{font-size:7pt;}</style>');
...
if($jml_infoUjiKelayakan==0){
	...
	echo'<div class="col-md-12 col-sm-12 col-xs-12">';
		...
		echo '<a href="print.php?nim='.$get_nim.'&id='.$id.'" class="btn btn-primary"><i class="fa fa-chevron-circle-left"></i> Kembali</a>';
...
<?php
	...
	for($halaman=1;$halaman<3;$halaman++) {
		$nomor=1;
		$noPenguji=1;
		echo '<h3>UNIVERSITAS DIPONEGORO <br> FAKULTAS SAINS DAN MATEMATIKA</h3><hr><br>';
		echo '<h3 align="center"><u>BERITA ACARA SEMINAR KELAYAKAN SKRIPSI</u></h3>';
		echo '<h4 align="right"> Lembar '.$halaman.' </h4><br>';
		...
		echo '<table>';
			...
			echo '<tr>';
				...
				echo '<td>:</td>';
				echo '<td>KIMIA/ S1 KIMIA/ '.$namaLab.'</td>';
			echo '</tr>';
			echo '<tr>';
		...
		echo '<table style="width:100%" border="1px">';
			...
			echo '<tbody>';
				...
				$infoPenguji=mysqli_query($con, "SELECT * from penguji_kelayakan inner join dosen on penguji_kelayakan.nip_penguji_kelayakan=dosen.nip where penguji_kelayakan.id_uji_kelayakan='$id' and jabatan='penguji' ");
		...
		echo '<table class="ket_lembar" align="left" style="font-size:10pt">';
			echo '<tr>';
