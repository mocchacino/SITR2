	...
	if (!isset($_SESSION['sip_masuk_aja'])){
		...
		if ($_SERVER['REQUEST_METHOD'] === 'POST'){
			if (isset($_POST['submit'])) {
				$pass = mysqli_query($con, "SELECT * FROM dosen where nip='".$_SESSION['sip_masuk_aja']."' ");
				...
				if($validPassBaru && $validPassLama && $validPassBaruConfirm){
			  		$change_pass = mysqli_query($con,"UPDATE dosen SET password='".md5('sip'.$new_pass.'pis')."' where nip='".$_SESSION['sip_masuk_aja']."' ");
