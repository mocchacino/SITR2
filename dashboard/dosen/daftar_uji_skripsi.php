...
<script>
	...
	$(document).ready(function(){
		...
		$('#print').click(function(){
			...
			var kopsurat = '<body><h2 style="text-align:center;">DAFTAR TUGAS AKHIR<br>DEPARTEMEN KIMIA FAKULTAS SAINS DAN MATEMATIKA<br>UNIVERSITAS DIPONEGORO SEMARANG</h2><hr><hr><br> <script type="text/javascript" src="https://code.jquery.com/jquery-3.3.1.min.js"><\/script>';
	...
	<div class="col-md-12 col-sm-12 col-xs-12">
		<div class="panel panel-default">
			<div class="row panel-heading" style="margin: 0;">
				Daftar Mahasiswa yang Mendaftar Ujian Tugas Akhir
						...
						if ($_SERVER['REQUEST_METHOD'] === 'POST'){
							if(isset($_POST['search_tgl'])) {
								...
								if(empty($dari_tgl) && empty($sampai_tgl)){
									if(empty($periode)){
										$show_pendaftar="SELECT DISTINCT mahasiswa.nim, mahasiswa.nama, judul, nip1, nip2, nip3, jadwal, tempat,tahun_ajaran,id_daftar_skripsi from daftar_skripsi left join uji_skripsi on uji_skripsi.id_uji_skripsi=daftar_skripsi.id_daftar_skripsi inner join mahasiswa on mahasiswa.nim=daftar_skripsi.nim where (nip1='".$nip_saya."' or nip2='".$nip_saya."') ";
									}else{
										$show_pendaftar = "SELECT DISTINCT mahasiswa.nim, mahasiswa.nama, judul, nip1, nip2, nip3, jadwal, tempat,tahun_ajaran,id_daftar_skripsi from daftar_skripsi left join uji_skripsi on uji_skripsi.id_uji_skripsi=daftar_skripsi.id_daftar_skripsi inner join mahasiswa on mahasiswa.nim=daftar_skripsi.nim where daftar_skripsi.tahun_ajaran='$periode' and (nip1='".$nip_saya."' or nip2='".$nip_saya."')";
									}
								}else{
									$show_pendaftar = "SELECT DISTINCT mahasiswa.nim, mahasiswa.nama, judul, nip1, nip2, nip3, jadwal, tempat,tahun_ajaran,id_daftar_skripsi from daftar_skripsi left join uji_skripsi on uji_skripsi.id_uji_skripsi=daftar_skripsi.id_daftar_skripsi inner join mahasiswa on mahasiswa.nim=daftar_skripsi.nim where (uji_skripsi.jadwal BETWEEN '$dari_tgl' AND '$sampai_tgl') and (nip1='".$nip_saya."' or nip2='".$nip_saya."')";
								}
							}else{
								$show_pendaftar="SELECT DISTINCT mahasiswa.nim, mahasiswa.nama, judul, nip1, nip2, nip3, jadwal, tempat,tahun_ajaran,id_daftar_skripsi from daftar_skripsi left join uji_skripsi on uji_skripsi.id_uji_skripsi=daftar_skripsi.id_daftar_skripsi inner join mahasiswa on mahasiswa.nim=daftar_skripsi.nim where (nip1='".$nip_saya."' or nip2='".$nip_saya."')";
							}
							...
							while($rshow_pendaftar = $fshow_pendaftar->fetch_object()){
								...
								if ($rshow_pendaftar->nip1 == $nip_saya){
									...
									<br>
									<a href='edit_mhs_skripsi.php?nim=".$rshow_pendaftar->nim."&id=".$rshow_pendaftar->id_daftar_skripsi."' title='Edit'><i class='fa fa-pencil-square-o' aria-hidden='true' ></i></a>&nbsp;
									<br>
...
<script type="text/javascript">
	...
	$('#form_hapus').submit(function(e){
		...
		$.ajax({
			url:'delete.php',
			type:'get',
			data:'data=mahasiswa_uji_skripsi&id='+id+'&nim='+nim,
