				...
				if (isset($_POST['edit'])){
					...
					if ($sks_semester !=''){
						if (is_numeric($sks_semester)){
							if(($sks_semester >= 3)&&($sks_semester <= 24)){
								$validSks=TRUE;
							}else{
								$errorSks='sks berjalan minimal 3';
								$validSks=FALSE;
							}
						}else{
					...
					if($validPembimbing1&&$validPembimbing2&&$validNip&&$validJudul&&$validKomulatif&&$validSks){				
						...
						if(move_uploaded_file($_FILES['file']['tmp_name'], $target.$path)){
							$edit_pendaftar = mysqli_query($con,"UPDATE daftar_skripsi SET nip1='".$pembimbing1."',nip2='".$pembimbing2."',nip3='".$pembimbing3."',judul='".$judul."', sks_komulatif = '".$sks_komulatif."',sks_semester = '".$sks_semester."', path_file='".$path."' where  nim = '".$nim."' and id_daftar_skripsi='".$id."' " );
						...
						<div class="row">
							<div class="col-md-12">
								...
								<div class="row">
									...
									<div class="form-group col-md-6 col-sm-6 col-xs-12">
										<input class="form-control" type="text" id="nama" name="nama" readonly value="<?php echo $nama;?>" >
									</div>
								</div>
