...
setlocale(LC_ALL, 'id_ID');
	...
	if (!isset($_SESSION['sip_masuk_aja'])){
		...
		$get_nim=$_GET['nim'];
		$id=$_GET['id'];
		$show_pendaftar = mysqli_query($con,"SELECT nama, daftar_skripsi.nim, judul, nip1, nip2, nip3 FROM daftar_skripsi INNER JOIN mahasiswa on mahasiswa.nim=daftar_skripsi.nim where daftar_skripsi.id_daftar_skripsi='$id' limit 1 ");
...
<?php
	...
	for($i=1;$i<10;$i++){
		...
		echo '<table>';
			...
			echo '</tr>';
			echo '<tr>';
				echo '<td>DEPARTEMEN/ PROGRAM STUDI</td>';
				echo '<td>:</td>';
		...
		echo '<table class="isi" style="width:100%" border="1px">';
			...
			$nomor=1;
				...
				if($rshow_pendaftar['nip3']!=''){
					$infoPembimbing3=mysqli_query($con, "SELECT * from pembimbing_luar where nip='".$rshow_pendaftar['nip3']."' ");
					$rInfoPembimbing3=$infoPembimbing3->fetch_object();
					echo '<tr>';
					echo '<td align="center">'.$nomor.'</td>'; 
					echo '<td>'.$rInfoPembimbing3->nama.'</td>';
					echo '<td>PEMBIMBING '.$nomor.'</td>';
					echo '<td>'.$nomor.'</td>';
					echo '<td></td>';
					echo '</tr>';
