<?php	
	...
	if (!isset($_SESSION['sip_masuk_aja'])){
		...
		if ($_SERVER['REQUEST_METHOD'] === 'POST'){
			//$nim='24010314120014';
			$id = $_POST['id'];
			$query = mysqli_query($con,"SELECT mahasiswa.nama, jadwal, tempat, nip1, nip2, nip3, judul,uji_skripsi.nim FROM daftar_skripsi inner join mahasiswa on mahasiswa.nim=daftar_skripsi.nim inner join uji_skripsi on uji_skripsi.nim=mahasiswa.nim inner join penguji_skripsi on penguji_skripsi.id_uji_skripsi=uji_skripsi.id_uji_skripsi where uji_skripsi.id_uji_skripsi='$id' and uji_skripsi.is_lulus='0' limit 1 ");
			$row = $query->fetch_row();
			...
			$pembimbing3=mysqli_query($con, "SELECT nip, nama from pembimbing_luar where pembimbing_luar.nip='$row[5]' ");
